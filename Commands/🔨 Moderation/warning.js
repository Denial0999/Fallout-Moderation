...
module.exports = {
    category: "warnings",
    description: "Set a users warning",
    ...
    callback: async ({client, interaction}) => {
        ...
        if(Sub === "add"){
            db.findOne({ GuildID: guildId, UserID: Target.id, UserTag: Target.user.tag}, async (err, data) => {
                if(err) return interaction.reply("An error occured while trying to find the user.");
                if(!data){
                    data = new db({
                        GuildID: guildId, 
                        UserID: Target.id, 
        ...
        try {
            await Target.send({embeds: [new MessageEmbed().setColor("#00defa").setTitle("⚠️ WARNING").setAuthor(Target.user.tag, Target.user.avatarURL({dynamic: true, size: 512})).setDescription(`You have been warned for: \`\`\`${Reason}\`\`\` \nServer Name: **${guild.name}**`).setFooter(`ID: ${Target.user.id}`)]});
        } catch (e) {
            return;
        } 
